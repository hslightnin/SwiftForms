...
open class FormOptionsSelectorController: UITableViewController, FormSelector {
    ...
    open override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let _ = selectedOptions.firstIndex(where: { $0 as! AnyHashable == optionValue as! AnyHashable }) {
    ...
    open override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if allowsMultipleSelection {
            if var selectedOptions = formCell?.rowDescriptor?.value as? [AnyObject] {
                if let index = selectedOptions.firstIndex(where: { $0 as! AnyHashable == selectedOption as! AnyHashable }) {
